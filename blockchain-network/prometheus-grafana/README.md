...
This sample provides an environment to display and capture metrics from the blockchain-network in real time. It consists of a docker-compose file that starts a Prometheus and Grafana server setup configured to collect and display metrics for the test network.
...
You will need to have installed **docker-compose with version 1.29 or above** (note that this is higher than the v1.14 requirement requested for the blockchain-network).

## How to use

1. Go to the blockchain-network directory and run bring up the blockchain-network **./network.sh up createChannel**
2. Bring up the Prometheus/Grafana network in the blockchain-network/prometheus-grafana directory and run **docker-compose up -d**
...
5. Deploy a chaincode (i.e. "./network.sh deployCC -ccn basic -ccp ../asset-transfer-basic/chaincode-go -ccl go"), start using the blockchain-network and use the Grafana dashboard to analyse and assess your network performances.
